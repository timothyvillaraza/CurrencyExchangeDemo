...
public class CurrencyExchangeViewModel
{
    ...
    private IEnumerable<SelectListItem> GetAllCurrencies()
    {
        // TODO: Fetch list of currencies dynamically with an API call
        ...
        var orderedCurrencies = currencies.OrderBy(c => c.Key != "usd") // USD comes first
            .ThenBy(c => string.IsNullOrWhiteSpace(c.Value) ? c.Key : c.Value) // Sort by value or key if value is blank
            .Select(c => new SelectListItem
            {
                Value = c.Key,
                Text = string.IsNullOrWhiteSpace(c.Value) ? c.Key.ToUpper() : c.Value // Use key if value is blank
            })
            .ToList();
